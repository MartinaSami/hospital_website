...
<div class="container col-sm-3">
    <div class="toast" data-autohide="false">
        <div class="toast-header">
            <strong class="mr-auto text-primary" style='font-size:22px;'> Don’t Forget Reservation In <strong
                    class="mr-auto text-primary" style='font-size:25px;'><?php echo $q ;?></strong> </strong>
            <small class="text-muted"> </small>
            <button type="button" style='font-size:55px;' class="ml-6 mb-1 close text"
            ...
            <?php
                    ...
                    if ($count > 0) {
                        foreach ($row as $r) {
                            ...
                            $stmt = $db->prepare("SELECT doctor_name, time_in FROM  doctor WHERE doctor_id=$c");
                            $stmt->execute();
                            $row = $stmt->fetchAll();
                            $count = $stmt->rowCount();
                            if ($count > 0) {
    ...
    <form action="Home_user.php?action=take" method="POST">
        <div class="card">
            <div class="card-header" style="background-color: #173753;">
                <h5 class="text-center" style="font-size: 40px ; color: white;">Select Doctor </h5>
            </div>
                ...
                <div class="row">
                    <div class="col-sm-4">
                        ...
                        <select class="custom-select custom-select-lg" name="specialization" id="f">
                            <?php
                                    $stmt = $db->prepare("SELECT Specialization_name,Specialization_id FROM specialization1 ");
                                    $stmt->execute();
                                    $row = $stmt->fetchAll();
                                    $count = $stmt->rowCount();
                                    if ($count > 0) {
                                        ...
                                        foreach ($row as $use) {
                                            echo "<option value='". $use['Specialization_id']."'>". $use['Specialization_name']  . "</option>";
                                        }
                                    }
                                    ?>
                        </select>
                    ...
                    <div class="col-sm-4">
                        <div class="form-group">
                            <label class="req" style="font-size: 30px ;color: white;">Doctor
                                Name</label>
                            <select class="custom-select custom-select-lg" name="doctor" id="s">
                                <?php
                                        $sp=$_SESSION['ID'];
                                        $GLOBALS['iddoc']= '';
